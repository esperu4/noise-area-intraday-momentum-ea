...
int SessionStart() { return InpSessionStartHour*60+InpSessionStartMinute; }
int SessionEnd() { return InpSessionEndHour*60+InpSessionEndMinute; }
datetime NthSunday(int year,int month,int ordinal) { MqlDateTime x; x.year=year; x.mon=month; x.day=1; x.hour=0; x.min=0; x.sec=0; datetime first=StructToTime(x); MqlDateTime y; TimeToStruct(first,y); return first+((7-y.day_of_week)%7+7*(ordinal-1))*86400; }
bool UsDst(datetime utc) { MqlDateTime x;TimeToStruct(utc,x); datetime start=NthSunday(x.year,3,2)+7*3600, finish=NthSunday(x.year,11,1)+6*3600; return utc>=start&&utc<finish; }
...
int OnInit()
{
 ...
 trade.SetExpertMagicNumber(InpMagicNumber); trade.SetDeviationInPoints(InpDeviationPoints); trade.SetTypeFillingBySymbol(_Symbol); dayKey=DateKey(ServerToNewYork(TimeCurrent())); dayBaseline=AccountInfoDouble(ACCOUNT_EQUITY); managed.valid=false;
 PrintFormat("EVENT=START ENABLED=%s SYMBOL=%s LOOKBACK=%d INTERVAL=%d SESSION=%02d:%02d-%02d:%02d_NEW_YORK MAGIC=%I64d",InpStrategyEnabled?"true":"false",_Symbol,InpNoiseLookback,InpSignalIntervalMinutes,InpSessionStartHour,InpSessionStartMinute,InpSessionEndHour,InpSessionEndMinute,InpMagicNumber); PrintFormat("EVENT=SYMBOL_INFO DIGITS=%d TICK_SIZE=%f TICK_VALUE=%f VOL_MIN=%f VOL_STEP=%f SPREAD_POINTS=%d AUTO_OFFSET=%s",(int)SymbolInfoInteger(_Symbol,SYMBOL_DIGITS),SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE),SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_VALUE),SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN),SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP),(int)SymbolInfoInteger(_Symbol,SYMBOL_SPREAD),InpUseAutoServerUtcOffset?"true":"false"); Print("WARNING=AUTOMATED_STRATEGY VERIFY_TIMEZONE_SYMBOL_VOLUME_SPREAD_RISK"); return INIT_SUCCEEDED;
}
...
{
 MqlRates r[]; int n=CopyRates(_Symbol,PERIOD_M1,0,MathMax(5000,(InpNoiseLookback+InpWarmupExtraSessions)*500),r); if(n<=0)return 0; int count=0; for(int i=0;i<n;i++){datetime ny=ServerToNewYork(r[i].time);if(DateKey(ny)>=DateKey(currentNy)||MinutesOf(ny)<SessionStart()||MinutesOf(ny)>=SessionEnd())continue;int key=DateKey(ny);bool found=false;for(int j=0;j<i;j++){datetime p=ServerToNewYork(r[j].time);if(DateKey(p)==key){found=true;break;}}if(!found)count++;}return count;
}
NoiseBand BuildNoise(datetime currentNy)
{
 ...
 int curMinute=MinutesOf(currentNy)-SessionStart(); if(curMinute<=0)return out; int keys[];double opens[],closes[];ArrayResize(keys,0);ArrayResize(opens,0);ArrayResize(closes,0);
 for(int i=0;i<n;i++){datetime ny=ServerToNewYork(r[i].time);int key=DateKey(ny),m=MinutesOf(ny);if(key>=DateKey(currentNy)||m<SessionStart()||m>=SessionEnd())continue;int pos=-1;for(int j=0;j<ArraySize(keys);j++)if(keys[j]==key){pos=j;break;}if(pos<0){pos=ArraySize(keys);ArrayResize(keys,pos+1);ArrayResize(opens,pos+1);ArrayResize(closes,pos+1);keys[pos]=key;opens[pos]=r[i].open;closes[pos]=r[i].close;}else closes[pos]=r[i].close;}
 if(ArraySize(keys)<InpNoiseLookback)return out; int first=ArraySize(keys)-InpNoiseLookback; double sum=0;int used=0; double prevClose=closes[ArraySize(keys)-1]; for(int j=first;j<ArraySize(keys);j++){int target=-1;for(int i=0;i<n;i++){datetime ny=ServerToNewYork(r[i].time);if(DateKey(ny)==keys[j]&&MinutesOf(ny)-SessionStart()==curMinute){target=i;break;}}if(target>=0){sum+=MathAbs(r[target].close/opens[j]-1.0);used++;}}
 if(used<InpNoiseLookback)return out; double sessionOpen=0;for(int i=0;i<n;i++){datetime ny=ServerToNewYork(r[i].time);if(DateKey(ny)==DateKey(currentNy)&&MinutesOf(ny)>=SessionStart()&&MinutesOf(ny)<SessionEnd()){sessionOpen=r[i].open;break;}}if(sessionOpen<=0)return out;out.sigma=sum/used;double ua=MathMax(sessionOpen,prevClose),la=MathMin(sessionOpen,prevClose);out.upper=ua*(1+InpNoiseMultiplier*out.sigma);out.lower=la*(1-InpNoiseMultiplier*out.sigma);out.valid=true;return out;
}

double SessionVWAP(datetime currentNy)
{
 MqlRates r[];int n=CopyRates(_Symbol,PERIOD_M1,0,1000,r);double pv=0,v=0;for(int i=0;i<n;i++){datetime ny=ServerToNewYork(r[i].time);if(DateKey(ny)!=DateKey(currentNy)||MinutesOf(ny)<SessionStart()||MinutesOf(ny)>=SessionEnd())continue;double q=MathMax(0.0,r[i].tick_volume);pv+=((r[i].high+r[i].low+r[i].close)/3.0)*q;v+=q;}return v>0?pv/v:0;
}
double AtrValue()
{
...
double CalculateVolume(ENUM_ORDER_TYPE orderType,double entry,double stop)
{
 if(InpSizingMode==SIZING_FIXED_VOLUME)return VolumeFloor(InpFixedVolume);double cashRisk=AccountInfoDouble(ACCOUNT_EQUITY)*InpRiskPercent/100.0;double tickSize=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_SIZE),tickValue=SymbolInfoDouble(_Symbol,SYMBOL_TRADE_TICK_VALUE);double lossPerLot=0;if(tickSize>0&&tickValue>0)lossPerLot=MathAbs(entry-stop)/tickSize*tickValue;else{double oneLotProfit=0;if(!OrderCalcProfit(orderType,_Symbol,1.0,entry,stop,oneLotProfit))return 0;lossPerLot=MathAbs(oneLotProfit);}return lossPerLot>0?VolumeFloor(cashRisk/lossPerLot):0;
}
